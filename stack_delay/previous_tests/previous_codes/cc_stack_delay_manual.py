...
for v in range (nov):
    ...
    for s in range(nos):
      ...
      if distdeg < distlim and distdeg > 0.01:
         try:
            st += client.get_waveforms(network="AK", station=net[s].code, location="*", channel="BHZ", starttime=s1+tt-starttime, endtime=s1+tt+endtime-0.01, attach_response=True) #call waveforms that fit the distance criteria, perhaps this step can be avoided in next runs. 
    ...
    ststack = np.mean(data, axis=0)    
    maxpower = np.max(np.abs(ststack)) #max of the abs value of the stack
    stack_power = np.append(stack_power, [maxpower]) #put maxpowers into an array
    ####ststackenv = obspy.signal.filter.envelope(ststack) #calculate env of each stack 
   ...
   #stack streams for each velocity
    ...
    ax3.plot(st[0].times(), ststack, "k-", linewidth=0.8, label='stream')
    ...
    ax3.set_ylabel('Displacement (km/sec)')
